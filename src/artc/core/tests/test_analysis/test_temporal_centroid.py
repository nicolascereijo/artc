...
def test_compare_two_temporal_centroid(setup: SetupData) -> None:
    data_set = setup
    ...
    assert (
        analysis.compare_two_temporal_centroid(
            audio_signal1, audio_signal1, sample_rate1, sample_rate1
        )
        == 1
    )
    assert (
        analysis.compare_two_temporal_centroid(
            audio_signal2, audio_signal2, sample_rate2, sample_rate2
        )
        == 1
    )
    assert (
        analysis.compare_two_temporal_centroid(
            audio_signal3, audio_signal3, sample_rate3, sample_rate3
        )
        == 1
    )
    assert (
        round(
            analysis.compare_two_temporal_centroid(
                audio_signal1, audio_signal2, sample_rate1, sample_rate2
            ),
            5,
        )
        == 0.72923
    ...
    assert (
        round(
            analysis.compare_two_temporal_centroid(
                audio_signal2, audio_signal1, sample_rate2, sample_rate1
            ),
            5,
        )
        == 0.72923
    ...
    assert (
        round(
            analysis.compare_two_temporal_centroid(
                audio_signal1, audio_signal3, sample_rate1, sample_rate3
            ),
            5,
        )
        == 0.75223
    ...
    assert (
        round(
            analysis.compare_two_temporal_centroid(
                audio_signal3, audio_signal1, sample_rate3, sample_rate1
            ),
            5,
        )
        == 0.75223
    ...
    assert (
        round(
            analysis.compare_two_temporal_centroid(
                audio_signal2, audio_signal3, sample_rate2, sample_rate3
            ),
            5,
        )
        == 0.51406
    ...
    assert (
        round(
            analysis.compare_two_temporal_centroid(
                audio_signal3, audio_signal2, sample_rate3, sample_rate2
            ),
            5,
        )
        == 0.51406
...
def test_compare_multiple_temporal_centroid(setup: SetupData) -> None:
    data_set = setup
    audio_signal1, sample_rate1 = load(
        data_set["individual_files"][0]["path"]
        / data_set["individual_files"][0]["name"]
    ...
    assert (
        analysis.compare_multiple_temporal_centroid(
            [audio_signal1, audio_signal1], [sample_rate1, sample_rate1]
        )
        == 1
    )
