...
def test_compare_two_zcr(setup: SetupData) -> None:
    data_set = setup
    ...
    assert (
        round(analysis.compare_two_zcr(audio_signal1, audio_signal2), 5)
        == 0.40036
    )
    assert (
        round(analysis.compare_two_zcr(audio_signal2, audio_signal1), 5)
        == 0.40036
    )
    assert (
        round(analysis.compare_two_zcr(audio_signal1, audio_signal3), 5)
        == 0.42543
    )
    assert (
        round(analysis.compare_two_zcr(audio_signal3, audio_signal1), 5)
        == 0.42543
    )
    assert (
        round(analysis.compare_two_zcr(audio_signal2, audio_signal3), 5)
        == 0.83951
    )
    assert (
        round(analysis.compare_two_zcr(audio_signal3, audio_signal2), 5)
        == 0.83951
    )


def test_compare_multiple_zcr(setup: SetupData) -> None:
    data_set = setup
    audio_signal1, _sample_rate1 = load(
        data_set["individual_files"][0]["path"]
        / data_set["individual_files"][0]["name"]
