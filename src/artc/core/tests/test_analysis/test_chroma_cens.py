...
def test_compare_two_chroma_cens(setup: SetupData) -> None:
    data_set = setup
    hop_length = 512
    ...
    assert (
        analysis.compare_two_chroma_cens(
            audio_signal1,
            audio_signal1,
            sample_rate1,
            sample_rate1,
...
def test_compare_multiple_chroma_cens(setup: SetupData) -> None:
    data_set = setup
    hop_length = 512
    audio_signal1, sample_rate1 = load(
        data_set["individual_files"][0]["path"]
    ...
    assert (
        analysis.compare_multiple_chroma_cens(
