...
def test_compare_two_tempo(setup: SetupData) -> None:
    data_set = setup
    hop_length = 512
...
def test_compare_multiple_tempo(setup: SetupData) -> None:
    data_set = setup
    hop_length = 512
    audio_signal1, sample_rate1 = load(
        data_set["individual_files"][0]["path"]
    ...
    assert (
        analysis.compare_multiple_tempo(
