...
def calculate_beat_alignment(
    ...
    *,
    ...
    """Detects beat positions in the audio signal using a beat tracking
    algorithm and returns the frequency domain representation of the beat
    activation sequence.
    ...
    Args:
        ...
        hop_length: Number of samples between successive analysis
            frames. Defaults to the 'beat_alignment' entry of
            '[metric.window_parameter]' in the TOML.

    Returns:
        FFT of the detected beat position sequence, or an empty array if no
        beat was detected.
    """
    if hop_length is None:
        hop_length = int(
            config.read_config(("window_parameter", "beat_alignment"))
        )
    _, beats = beat_track(
        y=audio_signal, sr=sample_rate, hop_length=hop_length
    )
    # np.fft.fft raises on empty input, no detectable pulse means zero
    # beats.
    if len(beats) == 0:
        return np.array([])
...
def compare_two_beat_alignment(
    ...
    *,
    ...
    """Compares beat alignment between two audio signals by computing their
    beat FFTs and calculating a normalized similarity score.
    ...
    beats_1 = calculate_beat_alignment(
        audio_signal1, sample_rate1, hop_length=hop_length
    )
    beats_2 = calculate_beat_alignment(
        audio_signal2, sample_rate2, hop_length=hop_length
    )

    # Only equal if neither signal has beats. One sided truncation below would
    # otherwise hide a real difference as a trivial match.
    ...
    distance = np.linalg.norm(
        np.abs(beats_1_adjusted) - np.abs(beats_2_adjusted)
    )
    max_distance = np.linalg.norm(
        np.abs(beats_1_adjusted)
    ) + np.linalg.norm(
...
def compare_multiple_beat_alignment(
    ...
    *,
    ...
    """Computes average beat alignment similarity for all unique signal pairs
    using 'compare_two_beat_alignment', reflecting overall rhythmic coherence.
